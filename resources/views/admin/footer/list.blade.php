...
@section('content')
    <div class="row-fluid">
        <div class="span12" style="margin-left: 0;">
            <a href="{{ url('/admin/footer-edit/0') }}">
                ...
                @if(count($row) > 0)
                    @foreach($main as $key => $item)
                        <tr class="row_{{$item->id}}">
                            ...
                            <td><a href="/admin/footer-edit/{{$item->id}}">{{$item->name_kz}}</a></td>
                            <td><a href="/admin/footer-edit/{{$item->id}}">{{$item->name_ru}}</a></td>
                            <td>@if($item->url)<a href="{{$item->url}}">Перейти</a>@endif</td>
                            <td>{{$item->order}}</td>
                            <td></td>
                        ...
                        <tr class="row_{{$s->id}}">
                            ...
                            <td><a href="/admin/footer-edit/{{$s->id}}">{{$s->name_kz}}</a></td>
                            <td><a href="/admin/footer-edit/{{$s->id}}">{{$s->name_ru}}</a></td>
                            <td>@if($s->url)<a href="{{$s->url}}">Перейти</a>@endif</td>
                            <td>{{$s->order}}</td>
                            <td>{{$item->name_ru}}</td>
                            <td style="padding-left: 20px;">
                                <div style="margin: 0 0 5px 0;">
