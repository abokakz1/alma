...
@section('content')
        ...
        <div class="tabbable">
            ...
            <div class="tab-content">
                <div class="tab-pane in active" id="main_data">
                    ...
                    @endif
                    <form id="myform" enctype="multipart/form-data" method="post" action="/admin/news-archive-edit/{{$row->news_id}}">
                        <div style="float: left; width: 900px;">
                            ...
                            <div class="left-block">
                                <p>
                                    Картинка в странице архива новостей<br>
                                    <span style="color: red;">Рекомендуемый размер картинки: 922х414</span>
                                </p>
                            </div>
                            <div class="right-block">
                                <div class="image-block2">
                                    <div>
                                        @if(strlen($row->image) > 0)
                                            @if($lang == "kz")
                                                <img src="/archive_news_kz/{{$row->image}}" width="150px" height="150px" id="news_image">
                                            @else
                                                <img src="/archive_news_ru/{{$row->image}}" width="150px" height="150px" id="news_image">
                                            @endif
                                        @else
                            ...
                            <div class="right-block tag-select-block" style="width: 535px;">
                                ...
                                $news_tag_list = ArchiveNewsTag::select("archive_news_tag_tab.*")->where("archive_news_tag_tab.news_id","=",$row->news_id)->get();
                                $i = 0;
                                ?>
                                ...
                                @if(count($news_tag_list) > 0)
                                    @foreach($news_tag_list as $key => $news_tag_item)
                                        <div class="tag-id-select" style="margin-bottom: 5px;">
                                            <select name="tag_id[{{$news_tag_item->archive_news_tag_id}}]" style="float: left; width: 178px; ">
                                            ...
                                            <i class="icon-remove red" style="float: left; margin: 7px 0px 0px 8px;" onclick="deleteTag({{$news_tag_item->archive_news_tag_id}},this)"></i>
                                            <div class="clearfloat"></div>
                                        </div>
                                    @endforeach
                                @endif
    ...
    <script>
        ...
        function deleteTag(archive_news_tag_id,ob){
            $(ob).closest(".tag-id-select").remove();

            if(archive_news_tag_id > 0){
                $.ajax({
                    type: 'GET',
                    url: "/admin/delete-archive-news-tag",
                    data: {_token: CSRF_TOKEN, id: archive_news_tag_id},
