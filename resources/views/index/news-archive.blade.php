    ...
    <div class="news v-tabs">
        ...
        <div class="mob-category hidden-lg hidden-md">
            <select onchange="setNewsByTypeMobile(this)" class="select-new-type">
                ...
                <option value="2">{{trans("messages.САМЫЕ ПОПУЛЯРНЫЕ")}}</option>
            </select>
        </div>
        <style>
                    ...
                    @foreach($row as $key => $news_item)
                        @if(strlen(trim($news_item['news_title_' . App::getLocale()])) > 0)
                            ...
                            <div class="col-lg-3 col-md-4 col-sm-6">
                                <div class="news-item">
                                    <a href="{{ LaravelLocalization::getLocalizedURL(App::getLocale(), "/news-archive/news/" . $news_item->news_url_name) }}">
                                        @if(strlen($news_item->image) > 0)
                                            @if(App::getLocale() == "kz")
                                                <div class="news-item-img op-darked" style="background-image: url('/archive_news_kz/{{$news_item->image}}')">
                                            @else
                                                <div class="news-item-img op-darked" style="background-image: url('/archive_news_ru/{{$news_item->image}}')">
                                            @endif
                                        @else
                                    ...
                                    <div class="news-item-title">
                                        <a href="{{ LaravelLocalization::getLocalizedURL(App::getLocale(), "/news-archive/news/" . $news_item->news_url_name) }}">{{ $news_item['news_title_' . App::getLocale()] }}</a>
                                    </div>
                                </div>
                            </div>
